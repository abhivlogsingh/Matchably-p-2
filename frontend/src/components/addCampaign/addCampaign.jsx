                    ...
                    {showDropdown && (
                      <div  className="w-full px-4 py-3 bg-black mt-[10px] text-white rounded-lg border border-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-600">
                        {formats.map((sectorName, index) => (
                          <label key={index} className="flex items-center space-x-2 cursor-pointer hover:bg-[#ffffff25] p-2 rounded-md transition-colors duration-200">
                          <input
                            type="checkbox"
                            value={sectorName}
                            checked={campaign.contentFormat.includes(sectorName)}
                            onChange={() => {
                              console.log(sectorName)
                              setCampaign(prev => ({
                                ...prev,
                                contentFormat: prev.contentFormat.includes(sectorName)
                                  ? prev.contentFormat.filter(format => format !== sectorName)
